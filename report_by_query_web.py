...
def home_page():
    ...
    in1 = jp.Input(a=wp, classes='form-input', value='(is_large_cap == True)', no_events=True)
    ...
    def submit_form(self, msg):
        try:
            ...
            for stock_report in report_data:
                ...
                table_data = [
                    ['Metric', 'Value'],
                    ['Last Close', "{:.2f} on {}".format(stock_report['last_close'], stock_report['last_close_date'])],
                ...
                for title, href in reporting.sites.items():
                    jp.Link(text=title, href=href.format(ticker),
                            classes="p-2 hover:underline text-blue-700 inline-flex items-center font-semibold tracking-wide",
                            target="_blank", a=d_in)
            jp.Br(a=wp)
            alert_div = jp.Div(classes="border-red-400 border m-2 p-2",
                               a=wp)
